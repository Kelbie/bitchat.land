...
export function useNostr(
  ...
  const connectToNostr = () => {
    try {
      ...
      // Get georelay-based relays for current channel (or global if no channel)
      const geoRelays = GeoRelayDirectory.shared.closestRelays(currentChannel || "u", 5);
      ...
      const sub = pool.subscribeMany(targetRelays, [{ kinds: [20000, 23333] }], {
        ...
        oneose() {
          ...
          setConnectedRelays(targetRelays);
        },
        onclose() {
